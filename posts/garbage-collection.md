...
mark-and-sweep의 퍼포먼스 문제로 인해, 현대의 tracing GC는 tri-color marking 의 추상적 모델을 기반으로 다양하게 발전하고 있습니다.
...
mark에서 접근 불가능한 객체가 발견되면, garbage collector가 해당 객체를 release하고, 나머지 객체들은 그대로 두거나 새로운 메모리 영역에 카피합니다. 
카피를 하는 경우, 필요에 따라 객체들의 참조를 갱신합니다.
...
참고가 생기면 count는 증가하고, 참조가 사라지면 감소합니다.
count가 0이 되었을때, 객체를 해제합니다.

## 장점
1. 객체가 접근 불가능해지는 즉시 메모리가 해제되므로, 프로그래머가 객체의 해제 시점을 어느정도 예측 가능합니다.
...
참조가 생성되고 제거될때마다 reference count를 증가,감소 시킨다면 퍼모먼스가 크게 하락할 것입니다.
